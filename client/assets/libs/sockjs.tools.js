var SockJsTools = {
    ...
    WebsocketRpc.prototype._bindSocketListeners = function() {
        ...
        this._onMessageProxy = function rpcOnMessageBoundFunction(event){
            if (self._socket.headers) {
                self._onMessage.apply(self, arguments);
            } else {
                self._onMessage.call(self, event.data);
            }
        };

        if (this._socket.headers) {
            this._socket.on('data', this._onMessageProxy);
        } else {
    ...
    /**
     * The SockJS socket already has an emitter mixin so steal it from there
     */
    WebsocketRpc.prototype._mixinEmitter = function() {
        var self = this;

        this._socket.on = this._socket.on || this._socket.addEventListener;
        this._socket.off = this._socket.off || this._socket.removeEventListener;

        if (typeof Backbone !== 'undefined') {
            _.extend(this, Backbone.Events);
            this.emit = this.trigger;
        } else {
            this.on = this.addEventListener = this._socket.on;
            this.off = this.removeEventListener = this._socket.off;
